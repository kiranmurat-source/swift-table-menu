...
function NutritionFactsTable({
  ...
  const rows: RowDef[] = [
    ...
    { label: UI.fat[uiLang], value: fat, unit: 'g', bold: true, ri: getRIPercent('fat', fat), tl: tlColor(getTrafficLight('fat', fat)) },
    { label: UI.saturates[uiLang], value: sat, unit: 'g', indent: true, ri: getRIPercent('saturatedFat', sat), tl: tlColor(getTrafficLight('saturatedFat', sat)) },
    { label: UI.carbs[uiLang], value: carbs, unit: 'g', bold: true, ri: getRIPercent('carbohydrate', carbs) },
    { label: UI.ofWhichSugars[uiLang], value: sugars, unit: 'g', indent: true, ri: getRIPercent('sugars', sugars), tl: tlColor(getTrafficLight('sugars', sugars)) },
    { label: UI.protein[uiLang], value: protein, unit: 'g', bold: true, ri: getRIPercent('protein', protein) },
    { label: UI.salt[uiLang], value: salt, unit: 'g', bold: true, ri: getRIPercent('salt', salt), tl: tlColor(getTrafficLight('salt', salt)) },
  ];
...
/* ------------------------------------------------------------------ */
/*  Nutri-Score Badge + Dropdown                                       */
/* ------------------------------------------------------------------ */

function NutriScoreBadge({
  score,
  lang,
  theme,
  size = 20,
    ...
    <>
      ...
      {open && (
        <>
          ...
          >
            <div style={{ fontSize: 13, fontWeight: 700, color: theme.text }}>{UI.nutriScoreTitle[uiLang]}</div>
            <div style={{ fontSize: 11, color: theme.mutedText, marginTop: 2 }}>
              {UI.nutriScoreDesc[uiLang]}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: theme.mutedText, marginTop: 4 }}>
              <span>{UI.nutriScoreHealthier[uiLang]}</span>
              <span>{UI.nutriScoreLessHealthy[uiLang]}</span>
            </div>
